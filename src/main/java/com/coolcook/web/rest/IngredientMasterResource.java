...
public class IngredientMasterResource {
    ...
    /**
     * SEARCH  /_search/ingredient-masters?query=:query : search for the ingredientMaster corresponding
     * to the query.
     *
     * @param query the query of the ingredientMaster search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/ingredient-masters")
    @Timed
    public ResponseEntity<List<IngredientMasterDTO>> searchIngredientMasters(@RequestParam String query, @ApiParam Pageable pageable) {
        log.debug("REST request to search for a page of IngredientMasters for query {}", query);
        Page<IngredientMasterDTO> page = ingredientMasterService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/ingredient-masters");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
