...
public class RecipeMasterResource {
    ...
    /**
     * SEARCH  /_search/recipe-masters?query=:query : search for the recipeMaster corresponding
     * to the query.
     *
     * @param query the query of the recipeMaster search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/recipe-masters")
    @Timed
    public ResponseEntity<List<RecipeMasterDTO>> searchRecipeMasters(@RequestParam String query, @ApiParam Pageable pageable) {
        log.debug("REST request to search for a page of RecipeMasters for query {}", query);
        Page<RecipeMasterDTO> page = recipeMasterService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/recipe-masters");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
